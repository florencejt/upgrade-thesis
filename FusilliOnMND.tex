...
\begin{table}
\centering
\caption{The eight fusion models used for predicting survival categories in motor neuron disease patients, implemented in Fusilli for combining two tabular modalities.}
...
\begin{table}[h]
\centering
\caption[Performance metrics on predicting short versus long survival in motor neuron disease patients from both datasets: ALS Biomarkers Study and Ospedale San Raffaele.]{Means and standard deviations of metrics calculated on the held-out test sets of the stability repetitions. Data from ALS Biomarkers Study and Ospedale San Raffaele was pooled to form the train and test sets. The best performing model within that metric is bolded. \textbf{Acronyms:} AUROC = area under the receiver operating characteristic, VAE = variational autoencoder. } 
...
Another aspect of the small sample size was the lack of a completely external test set.
