...
\begin{figure}
    ...
    \textbf{A}: If the user chooses to save the training logs as figures, as opposed to logging the losses with an online tool, this figure shows the training and validation loss.\\
    \textbf{B}: End-of-training evaluation of a model which was trained on a binary task with 5-fold cross validation. The performance of each fold is shown, as well as the overall performance from the aggregated validation folds. \\
    \textbf{C}: Comparison of the models' performances on the validation data. The user can choose which metrics to compare the models, and can also choose to compare the models on an external test set. The violin plot distributions are the distributions of the fold performances when using k-fold cross validation. If train-test split training is used, the comparison figure is a bar chart. \\
    }
