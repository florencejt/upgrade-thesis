...
Before designing Fusilli, the following design goals were set to ensure the package would be useful for a wide range of users and tasks, as well as for my own research.\newline
...
The only input that Fusilli requires is the user's data, what task they want to perform, and what models they want to compare.

\noindent\textbf{Experiment setup}: The experiment setup is the part of Fusilli that allows the user to define the experiment they want to run.
The user defines:
\begin{itemize}
    \item The task they want to perform (regression or classification).
    \item Their data.
    \item The model (or models) they want to train.
    \item Any hyperparameters or model architectures they want to change.
    \item Experiment specifics, such as training and validation data splits, and the number of epochs to train for.
\end{itemize}

\noindent\textbf{Data preparation}: Different models may require different data preparation steps.
